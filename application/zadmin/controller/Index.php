...
class Index extends BaseAdmin
  {
   public  function index(){
        ...
        //会员数量
        $user_cou=db("user")->where("is_dell=1")->count();
        $this->assign("user_cou",$user_cou);

        $today=db("user")->where("is_dell=1")->whereTime("u_ztime","week")->count();
        $this->assign("today",$today);

        $month=db("user")->where("is_dell=1")->whereTime("u_ztime","month")->count();
